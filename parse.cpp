...
void Parse(Render_World& world,int& width,int& height,const char* test_file)
{
    ...
    while(fgets(buff, sizeof(buff), F))
    {
        ...
        else if (item == "cubed_sphere") {
            // u is center, f0 is radius
            ss >> u >> f0 >> voxel_size >> mat;
            assert(ss);
            CubedSphere *s = new CubedSphere(u, f0, voxel_size);
            finish_parse_object(s);
        }
        else if(item=="mesh")
        {
            ss>>s0>>mat;
            ...
            Mesh* m=new Mesh;
            m->Read_Obj(s0.c_str());
            finish_parse_object(m);
        }
        else if (item=="cubed_mesh") {
            ss>>s0>>voxel_size>>mat;
            assert(ss);
            Mesh* m=new Mesh;
            m->Read_Obj(s0.c_str());
            m->Generate_Mesh_From_Density_Field(
                m->Generate_Density_Field_From_Mesh(voxel_size), 0.5f,
                m->Bounding_Box(-1).lo);
            finish_parse_object(m);
        }
